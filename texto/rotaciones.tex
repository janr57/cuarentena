...
exponencial, con módulo $r$ y argumento $\alpha$,
$z=e^{i\alpha}$ (ver figuras~\ref{fig:rot-z} y~\ref{fig:rot-z-rotado}),
lo que también permite  deducir que el módulo $r$ del complejo $z$
se conserva en la rotación
...
Lo anterior permite deducir también que el módulo $r$ del complejo $z$ se
conserva en la rotación.
\begin{figure}[ht]
  \centering
...
%        fill=\backgroundcolor, fit= (O) (letraejex) (letraejey)] {};
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Número complejo $z$ de módulo $r$ y argumento $\alpha$ en el
      plano complejo.}
    \label{fig:rot-z}
  \end{minipage}
  \hspace{1em}
...
%        fill=\backgroundcolor, fit= (O) (letraejex) (letraejey)] {};
    ...
    \caption{Vector $z=re^{i\alpha}$ rotado un ángulo $\theta$ al
      multiplicarlo por $e^{i\theta}$. Obsérvese que el módulo $r$ se
      mantiene constante.}
    \label{fig:rot-z-rotado}
  \end{minipage}
\end{figure}

Ahora escribimos el complejo con su parte real e imaginaria, $z=x+iy$
\[
  x'+iy'
  = e^{i\theta} (x+iy)
...
\begin{align*}
  ...
  &=
    ...
    \right)\mmm{A}
\end{align*}
y obtenemos una relación que se comparara con la identidad de
...
Al final obtenemos
\begin{equation}\label{eq:rot-ethetaA}
  e^{\theta\!\mmm{A}}
  = \begin{pNiceMatrix}
    \cos\theta & -\sin\theta\\
    \sin\theta & \cos\theta
  \end{pNiceMatrix}
\end{equation}
y llegamos a la conclusión de que ambas exponenciales describen una
rotación en dos dimensiones. Una de ellas, $e^{i\theta}$, rota números
complejos en el plano complejo y la otra, $e^{\theta\!\mmm{A}}$, vectores en el
plano euclídeo $\symbb{R}^2$
\[
...
el resultado dependería del parámetro $\eta$.
